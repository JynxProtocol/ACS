...
string  stance      = ")<SF>(  Two Hand Sword Ready Stance";
...
hit(string type, string anim, string sound)
{
    ...
    pos     = llGetPos() + vecAimOffset + fwd*1.3;
    ...
    if (Stamina < 0)
    {
         ...
         //llSay(acsChannel,"atcoaesd03");
         llSay(acsChannel,(encrypt(stamina)));
         Stamina = MStamina;
    }   
    
    
    if (strike == "Two Hand Hammer 2" || strike == "Two Hand Hammer 1")  
    {
...
init()
{
    //llSetText("Demo ACS Weapon",<1.0,1.0,1.0>, 1);
...
default
{
    ...
    listen(integer chan, string name, key id, string msg)
    {
        ...
        if (llGetOwnerKey(id) == llGetOwner())
        {
            ...
            else if (msgLower == "hammer sheath")
            {
                if (armed ==TRUE)
                {
