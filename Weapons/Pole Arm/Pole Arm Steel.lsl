...
string stamina = "atcoaesd01";

// ACS - QuaterStaff Script
// 
// Created by Harm Gulick
//
// This script contains the Avilion Combat System Sword Script
//
// Version  1.2.0
...
string  stance      = ")<SF>(  Polearm Ready Stance";
...
float       durationType1and2 =  0.6;   
float       durationOther =  0.85;      
integer     permissions;
integer     armed;                      
...
hit(string type, string anim, string sound)
{
    ...
    if (strike == "Polearm 2" || strike == "Polearm 1 (Steel)")  
    {
...
default
{
    ...
    control(key id, integer held, integer change) 
    {
        ...
        if (held & (CONTROL_ML_LBUTTON | CONTROL_LBUTTON))
        {
            ...
            {
                hit("Polearm 1 (Steel)", "startforwardattack", "");
            }
            else if (pressed  & (CONTROL_LEFT | CONTROL_ROT_LEFT)) 
            {
                hit("Polearm 1 (Steel)", "startleftattack", "");
            }
            else if (pressed  & (CONTROL_RIGHT | CONTROL_ROT_RIGHT))
            {
                 hit("Polearm 1 (Steel)", "startrightattack", "");
            }
            else if (pressed  & CONTROL_BACK)
            {
                 hit("Polearm 2", "startblock", "");
            }
